...
/*   Updated: 2022/03/09 13:21:44 by ntan             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((map->p_pos.x == map->m_pos.x
			|| (map->c >= 1 && map->f_pos.x == map->m_pos.x))
		&& (map->p_pos.y == map->m_pos.y - 1
			|| map->p_pos.y == map->m_pos.y + 1
			|| (map->c >= 1 && map->m_pos.y == map->f_pos.y - 1)
			|| (map->c >= 1 && map->m_pos.y == map->f_pos.y + 1)))
		map->end_status = 1;
	else if ((map->p_pos.y == map->m_pos.y
			|| (map->c >= 1 && map->f_pos.y == map->m_pos.y))
		&& (map->p_pos.x == map->m_pos.x - 1
			|| map->p_pos.x == map->m_pos.x + 1
			|| (map->c >= 1 && map->m_pos.x == map->f_pos.x + 1)
			|| (map->c >= 1 && map->m_pos.x == map->f_pos.x - 1)))
		map->end_status = 1;
}
